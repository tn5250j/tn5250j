...
public class Screen5250 implements TN5250jConstants {
	...
	public final void copyMe(Rectangle area) {
		...
		while (workR.height-- > 0) {
			...
			while (t-- > 0) {
				...
				if (c >= ' '
						&& (planes.screenExtended[getPos(m - 1, i - 1)] & EXTENDED_5250_NON_DSP)
	...
	public char[] getData(int startRow, int startCol, int endRow, int endCol,
			int plane) {
	...
	public synchronized int GetScreen(char buffer[], int bufferLength,
			int from,
...

	{
		...
		return planes.GetScreenRect(buffer, bufferLength, startPos, endPos,
				plane);
...

	{
		...
		return planes.GetScreenRect(buffer, bufferLength, startRow, startCol,
				endRow,
	...
	int right, int ll, int bottom, int lr) {
		...
		while (w-- >= 0) {
			...
			planes
					.setScreenCharAndAttr(lastPos, (char) upper, colorAttr,
							false);
		...
		while (depth-- > 0) {
			...
			planes
					.setScreenCharAndAttr(lastPos, (char) right, colorAttr,
							false);
		...
		while (w-- >= 0) {
			...
			planes.setScreenCharAndAttr(lastPos, (char) bottom, colorAttr,
					false);
	...
	protected void restoreErrorLine() {
		...
		if (planes.isErrorLineSaved()) {
			...
			fireScreenChanged(1, planes.getErrorLine() - 1, 0, planes
					.getErrorLine() - 1, numCols - 1);
		...
		case STATUS_SYSTEM:
			...
			if (value == STATUS_VALUE_ON) {
				...
				oia.setInputInhibited(ScreenOIA.INPUTINHIBITED_SYSTEM_WAIT,
						ScreenOIA.OIA_LEVEL_INPUT_INHIBITED, s);
				...
				oia.setInputInhibited(ScreenOIA.INPUTINHIBITED_NOTINHIBITED,
						ScreenOIA.OIA_LEVEL_NOT_INHIBITED, s);
	...
	private synchronized void fireCursorChanged(int update) {
		...
		if (listeners != null) {
			...
			for (int i = 0; i < size; i++) {
				...
				target.onScreenChanged(update, startRow, startCol, startRow,
						startCol);

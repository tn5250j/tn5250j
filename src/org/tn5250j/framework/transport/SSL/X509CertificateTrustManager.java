...
public class X509CertificateTrustManager implements X509TrustManager {
    ...
    public X509CertificateTrustManager(final TrustManager[] managers, final KeyStore keyStore) {
    ...
    public void checkServerTrusted(final X509Certificate[] chain, final String type)
        ...
        try {
            ...
            if (!UiUtils.showYesNoConfirm(certInfo, "Accept Certificate")) {
    ...
    public X509Certificate[] getAcceptedIssuers() {
        final ArrayList<X509Certificate> list = new ArrayList<X509Certificate>(10);
        for (int i = 0; i < trustManagers.length; i++) {
            if (trustManagers[i] instanceof X509TrustManager)
